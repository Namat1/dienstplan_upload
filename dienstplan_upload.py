...
# --- HIER DEINE FESTE FAHRERLISTE EINTRAGEN ---
# Für diese Fahrer wird IMMER eine Datei erstellt, auch wenn sie in der Excel fehlen.
STANDARD_FAHRER = [
    "Fechner, Klaus",
    "Fechner, Danny",
    "Scheil, Rene",
    ...
    "Lewandowski, Kamil",
    "Lewandowski, Dominik"
    # Du kannst hier beliebig viele weitere Fahrer exakt im Format "Nachname, Vorname" ergänzen!
]
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with ZipFile(zip_path, "w") as zipf:
                ...
                for file in uploaded_files:
                    ...
                    # 1. Globales Datum sicherer berechnen
                    valid_dates = pd.to_datetime(df.iloc[:, 14], errors="coerce").dropna()
                    if not valid_dates.empty:
                        global_start = pd.to_datetime(valid_dates.min())
                    else:
                        global_start = pd.Timestamp.today()
                    
                    global_start_sonntag = global_start - pd.Timedelta(days=(global_start.weekday() + 1) % 7)
                    global_kw = get_kw(global_start_sonntag) + 1

                    # 2. Standard-Fahrer vorab ins Dictionary laden (leere Pläne)
                    fahrer_dict = {fahrer: {} for fahrer in STANDARD_FAHRER}
                    
                    for _, row in df.iterrows():
                        # A) Alle Fahrer aus dieser Zeile auslesen
                        aktuelle_fahrer = []
                        for pos in [(3, 4), (6, 7)]:
                    ...
                    for fahrer_name, eintraege in fahrer_dict.items():
                        ...
                        if not eintraege:
                            ...
                            start_datum = pd.to_datetime(min(eintraege.keys()))
                        ...
                        for i in range(7):
                            ...
                            # Prüfe, ob Datum (als date-Objekt) in eintraege vorhanden ist
                            tag_date_only = tag_datum.date()
                            if tag_date_only in eintraege:
                                for eintrag in eintraege[tag_date_only]:
